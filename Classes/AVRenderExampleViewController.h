...
//  is invoked to generate a lossless movie that is a composite of 2 existing movies.
//  The result movie (.mvid file) is then played in the UI. The composition process is not really
//  fast (can take more than a minute on fast hardware) since each frame is generated and then
//  written to disk. The composite operation reads composition information from the Comp.plist file,
//  it then decodes the v1.mp4 and v2.mp4 files attached to the project to v1.mvid and v2.mvid.
//  The two movies are composed into a single movie using the specific timing and x,y data indicated
//  in the Comp.plist file. Finally, the composed movie is played in the GUI.
